...
file_lists AAM_Common::ScanNSortDirectory(const std::string &path, const std::string &extension)
{
	...
	searchPath = wstring(path.begin(), path.end()) + ws + wstring(extension.begin(), extension.end());
    ...
    do
    {
        ...
        else//if file
        {
			...
			string s = "/";
			searchFile = wstring(path.begin(), path.end()) + wstring(s.begin(), s.end()) + str;
			vFilenames.push_back(string(searchFile.begin(), searchFile.end()));
			nbFiles++;
		}
					  ...
					  bool showprocess /* = false */)
{
	...
	// for each level in the m_image pyramid
	//for (int iLev = startlev; iLev >= 0; iLev--)
    //{
		//printf("Level %d: ", 1);//printf("Level %d: ", iLev);

		IplImage* fitimage = 
			cvCreateImage(cvSize(w0/PyrScale, h0/PyrScale), image->depth, image->nChannels);
		cvResize(image, fitimage);
	
		__model[1]->Fit(fitimage, Shape, iter, showprocess);//__model[iLev]->Fit(fitimage, Shape, iter, showprocess);
